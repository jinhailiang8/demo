...
#else
static u8 icon_48x49_rgb565[4704] = {//format:r5g3 g3b5
...
static u8 icon_48x49_argb8565[7056] = {//format:a8 r5g3 g3b5
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
*/
/*----------------------------------------------------------------------------*/
void imb_create_ram_image_task(struct image_file *file, u8 *buf, int x, int y, int width, int height)
{
    struct imb_task_info imb_tsk_info = {0};
    struct imb_task *p = NULL;

    imb_tsk_info.data_src       = DATA_SRC_SRAM;//直接从ram取数
    imb_tsk_info.cur_in_flash   = 0;//数据存在ram
    imb_tsk_info.zip_en     = 0;
...
*/
/*----------------------------------------------------------------------------*/
void imb_create_ram_image_rotate_task(struct image_file *file, u8 *buf, int cx, int cy, int angle, int dx, int dy)
{
    struct imb_task_info imb_tsk_info = {0};
    struct imb_task *p = NULL;
...
*/
/*----------------------------------------------------------------------------*/
void imb_create_ram_image_scale_task(struct image_file *file, u8 *buf, float ratio_w, float ratio_h, int x, int y, int width, int height)
{
    struct imb_task_info imb_tsk_info = {0};
    struct imb_task *p = NULL;
...
*/
/*----------------------------------------------------------------------------*/
struct imb_task *imb_create_sram_image_scale_task(int image_width, int image_height, u8 *pixbuf, u8 format,  float ratio_w, float ratio_h, int x, int y, int width, int height)
{
    struct imb_task_info imb_tsk_info = {0};
    struct imb_task *p = NULL;
    ...
    imb_tsk_info.cur_in_flash   = 0;
    imb_tsk_info.zip_en         = 0;
    imb_tsk_info.in_format      = format;
    imb_tsk_info.x_offset       = x;
    imb_tsk_info.y_offset       = y;
    imb_tsk_info.src_w          = image_width;
    imb_tsk_info.src_h          = image_height;
    imb_tsk_info.des_w          = width;
    imb_tsk_info.des_h          = height;
...
static void imb_task_info_dump(struct imb_task_head *root)
{
    ...
    list_for_each_entry(p, &root->entry, entry) {
        printf("%d, %s, %d x %d, [%d, %d, %d, %d], %d\n", p->index, p->name, p->src_w, p->src_h, p->rect.left, p->rect.top, p->rect.width, p->rect.height, p->cur_format);
    }
...
void imb_image_scale(int bg_width, int bg_height, struct image_scale_info *scale)
{
    ...
    imb_create_sram_image_scale_task(scale->image_w, scale->image_h, scale->image, scale->format, scale->ratio_w, scale->ratio_h, scale->x, scale->y, scale->image_w * scale->ratio_w, scale->image_h * scale->ratio_h);
...
static void imb_task(void *arg)
{
    ...
    //从sram加载rgb565格式图片缩放测试 注意:每行字节数需4对齐,若不足4字节则补齐4字节
    float ratio_w = 1.5;
    float ratio_h = 1.5;
    int image_w = 48;
    int image_h = 49;
    imb_create_sram_image_scale_task(image_w, image_h, icon_48x49_rgb565, LAYER_FORMAT_RGB565, ratio_w, ratio_h, 100, 100, image_w * ratio_w, image_h * ratio_h);
    ...
    image_w = 48;
    image_h = 49;
    imb_create_sram_image_scale_task(image_w, image_h, icon_48x49_argb8565, LAYER_FORMAT_ARGB8565, ratio_w, ratio_h, 200, 100, image_w * ratio_w, image_h * ratio_h);
...
int imb_api_rotate(struct imb_api_info *info, struct imb_api_rotate_info *rot)
{
    ...
    if (rot->rotate_chunk) {
        imb_create_image_rotate_chunk_task(rot->file, &g_ui_resfile_info, rot->cx, rot->cy, rot->angle, rot->dx, rot->dy);
    } else {
        if (rot->buf) {
            imb_create_ram_image_rotate_task(rot->file, rot->buf, rot->cx, rot->cy, rot->angle, rot->dx, rot->dy);
        } else {
            imb_create_image_rotate_task(rot->file, &g_ui_resfile_info, rot->cx, rot->cy, rot->angle, rot->dx, rot->dy);
        }
    }
...
int imb_api_cut(struct imb_api_info *info, struct imb_api_cut_info *cut)
{
    ...
    if (cut->buf) {
        imb_create_ram_image_task(cut->file, cut->buf, cut->x, cut->y, cut->width, cut->height);
    } else {
        imb_create_image_task(cut->file, &g_ui_resfile_info, cut->x, cut->y, cut->width, cut->height);//width,height参数可小于file.width, file.height,若小于则裁剪
    }
...
int imb_api_sca(struct imb_api_info *info, struct imb_api_sca_info *sca)
{
    ...
    if (sca->buf) {
        imb_create_ram_image_scale_task(sca->file, sca->buf, sca->ratio_w, sca->ratio_h, sca->x, sca->y, sca->width, sca->height);
    } else {
        imb_create_image_scale_task(sca->file, &g_ui_resfile_info, sca->ratio_w, sca->ratio_h, sca->x, sca->y, sca->width, sca->height); //width/height参数小于放大后的宽高则为截取
    }
