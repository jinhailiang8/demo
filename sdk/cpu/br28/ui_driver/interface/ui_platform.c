...
#define imb_create_strpic() \
        ...
        if((file.format == TEXT_1BPP) || (file.format == PIXEL_FMT_L1) || ((file.format != TEXT_1BPP) && (file.compress == 1))) { \
...
static void *strpic_cb_in(void *priv)
{
    ...
    if (get_rect_cover(&head->curr_draw, &p->rect, &r) && p->cur_tsk_valid && !p->priv_buf[0]) {
        ...
        p->adr_map_adr = 0;
        u32 text_color = p->lut_tab[1];
        p->a_from_tsk = 0;
        p->r_from_tsk = ((text_color >> 11) & 0x1f) << 3;
        p->g_from_tsk = ((text_color >> 18) & 0x3f) << 2;
        p->b_from_tsk = ((text_color >> 27) & 0x1f) << 3;
...
static int br28_show_text(struct draw_context *dc, struct ui_text_attrs *text)
{
    ...
    if (text->format && !strcmp(text->format, "text")) {
        ...
        if (info && (FT_ERROR_NONE == (info->sta & (~(FT_ERROR_NOTABFILE | FT_ERROR_NOPIXFILE))))) {
            ...
            if (dc->align == UI_ALIGN_CENTER) {
                if (font_lang_get() == Arabic || font_lang_get() == Hebrew) { //显示方向从右到左
            ...
            if (p) {
                if (roll && (p->id == curr_id) && (p->priv_buf[0] != text->offset)) {
            ...
            if ((!p) || (p->id != curr_id) || p->scroll) {
                ...
                int len = font_text_out(text->encode, info, (u8 *)(text->str + roll * text->offset), text->strlen - roll * text->offset, x - dc->rect_ref.left, y - dc->rect_ref.top);
                text->displen = len;
