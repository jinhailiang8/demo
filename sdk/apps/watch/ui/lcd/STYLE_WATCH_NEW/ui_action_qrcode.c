...
static void upay_check_binding_time(void *priv)
{
    ...
    if (UPAY_STATUS_BINDING_OK == status) {
    ...
    } else if ((UPAY_STATUS_BINDING_FAIL == status) || (alipay_check_cnt > ALIPAY_CHECK_TIME_OUT_CNT)) {
        log_info("Bind FAIL!\n");
        ...
        if ((status >= UPAY_STATUS_GETTING_PROFILE) && (status <= UPAY_STATUS_FINISH_BINDING)) {
            log_info("Bind ing \n");
...
static void upay_check_pay_time(void *priv)
{
    ...
    if (alipay_check_cnt > ALIPAY_REFRESH_TIME_OUT_CNT) {
        alipay_check_cnt = 0;
        log_info("refresh\n");
        u32 msg_len = sizeof(alipay_QRcode_mess);
        upay_get_paycode(alipay_QRcode_mess, &msg_len);    //获取支付码
        ui_hide(UPAY_SHOW_BARCODE_LAYOUT);
        ui_hide(UPAY_SHOW_QRCODE_LAYOUT);
        if (alipay_show_barcode == 0) {
            ui_show(UPAY_SHOW_QRCODE_LAYOUT);
...
static int layout_alipay_bind_onchange(void *_ctrl, enum element_change_event event, void *arg)
{
    ...
    case ON_CHANGE_INIT:
        ...
        extern char *upay_get_version();
        printf("upay ver:%s \n", upay_get_version());
        upay_retval_t ret;
        ret = upay_env_init(UPAY_WATCH, TCFG_PAY_ALIOS_PRODUCT_MODEL, 2);    //环境初始化，此时状态应该为STATUS_START_BINDING
        printf("After init ,status is %02X, ret:%d \n", upay_get_binding_status(), ret);
        upay_ble_mode_enable(1);
        u32 msg_len = sizeof(alipay_QRcode_mess);
        ret = upay_get_binding_string(alipay_QRcode_mess, &msg_len);    //获取绑定码
...
{
    struct ui_text *text = (struct ui_text *)_ctrl;
    u32 len = sizeof(alipay_QRcode_mess);
    u8 *dat = alipay_QRcode_mess;
    switch (event) {
    case ON_CHANGE_INIT:
        printf("%s,%d \n", __func__, __LINE__);
        len --;
        upay_get_nick_name(dat, &len);
        if (len) {
            dat[len] = 0;
            put_buf(dat, len);
